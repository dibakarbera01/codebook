...
export const Checkout = ({ setcheckoutDisplay }) => {
  const { cartLists, total, clearCart } = useCart();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await createOrder(cartLists, total, user);
      clearCart();
      navigate("/orderpage", { state: { data: data, status: true } });
    } catch (error) {
      navigate("/orderpage", { state: { data: data, status: false } });
    }
  };
